...
Room.prototype.set_size_and_placement = function (room_holder_dimensions) {
  ...
  var location = [];
  location[0] = getRandomInt(
    Math.max(
      room_holder_dimensions[0] + MAX_ROOM_X / 2 - this.size[0] + 1,
      room_holder_dimensions[0]
    ),
    Math.min(
      room_holder_dimensions[0] + MAX_ROOM_X / 2 - 1,
      room_holder_dimensions[0] + MAX_ROOM_X - this.size[0]
    )
  );

  location[1] = getRandomInt(
    Math.max(
      room_holder_dimensions[1] + MAX_ROOM_Y / 2 - this.size[1] + 1,
      room_holder_dimensions[1]
    ),
    Math.min(
      room_holder_dimensions[1] + MAX_ROOM_Y / 2 - 1,
      room_holder_dimensions[1] + MAX_ROOM_Y - this.size[1]
    )
  );
  this.location = location;
};
...
Room.prototype.get_middle_of_room = function () {
  return [
    this.location[0] + Math.floor(this.size[0] / 2) + 1,
    this.location[1] + Math.floor(this.size[1] / 2) + 1,
  ];
};

Room.prototype.is_wall = function (loc) {
  if (
    loc[0] == this.location[0] ||
    loc[0] == this.location[0] + this.size[0] + 1 ||
    loc[1] == this.location[1] ||
    loc[1] == this.location[1] + this.size[1] + 1
  )
    return true;
  else {
    return false;
...
Room.prototype.is_door = function (loc) {
  var mid_grid_x =
    Math.floor(HOLDER_SIZE_X / 2) + GRID_DIMENSIONS[this.level_room_index][0];
  var mid_grid_y =
    Math.floor(HOLDER_SIZE_Y / 2) + GRID_DIMENSIONS[this.level_room_index][1];
...
Room.prototype.draw = function () {
  ...
  for (i = this.location[0]; i < this.location[0] + this.size[0] + 2; i++) {
    ...
    var args_b = convert_grid_location_into_filltext_args(
      i,
      this.location[1] + this.size[1] + 1
    );
    // if i is middle of room holder x:
    if (i == mid_grid_x) {
      if (this.north !== null) {
        // a room exists to north, paint a door.
        this.ctx.fillText("+", args_t[0], args_t[1]);
        if (this.north.visible) {
          for (
            path = this.location[1] - 1;
            path >= GRID_DIMENSIONS[this.level_room_index][1];
            path--
          ) {
            args_path = convert_grid_location_into_filltext_args(i, path);
      ...
      if (this.south !== null) {
        ...
        if (this.south.visible) {
          for (
            path = this.location[1] + this.size[1] + 2;
            path < GRID_DIMENSIONS[this.level_room_index][1] + HOLDER_SIZE_Y;
            path++
          ) {
            args_path = convert_grid_location_into_filltext_args(i, path);
            this.ctx.fillText("E", args_path[0], args_path[1]);
  ...
  for (i = this.location[1] + 1; i < this.location[1] + this.size[1] + 1; i++) {
    ...
    var args_e = convert_grid_location_into_filltext_args(
      this.location[0] + this.size[0] + 1,
      i
    );
    if (i == mid_grid_y) {
      if (this.west !== null) {
        ...
        if (this.west.visible) {
          for (
            path = this.location[0] - 1;
            path >= GRID_DIMENSIONS[this.level_room_index][0];
            path--
          ) {
            args_path = convert_grid_location_into_filltext_args(path, i);
            this.ctx.fillText("E", args_path[0], args_path[1]);
          }
        }
      } else {
      ...
      if (this.east !== null) {
        ...
        if (this.east.visible) {
          for (
            path = this.location[0] + this.size[0] + 2;
            path < GRID_DIMENSIONS[this.level_room_index][0] + HOLDER_SIZE_X;
            path++
          ) {
            args_path = convert_grid_location_into_filltext_args(path, i);
            this.ctx.fillText("E", args_path[0], args_path[1]);
  ...
  if (this.is_escape_room) {
    ...
    args = convert_grid_location_into_filltext_args(middle[0], middle[1]);
    this.ctx.fillText("O", args[0], args[1]);
  }

  if (DEBUG_SHOW_ROOM_INFO) {
    var location = this.location;
    args = convert_grid_location_into_filltext_args(
      location[0] + 1,
      location[1] + 1
    );
