...
Room.prototype.draw = function() {
        ...
        if (i == mid_grid_x) {
            if (this.north !== null) { // a room exists to north, paint a door.
                ...
                if (this.north.visible) {
                    for (path = this.location[1] - 1; path >= GRID_DIMENSIONS[this.level_room_index][1]; path--) {
            ...
            if (this.south !== null) {
                ...
                if (this.south.visible) {
                    for (path = this.location[1] + this.size[1] + 2; path < GRID_DIMENSIONS[this.level_room_index][1] + HOLDER_SIZE_Y; path++) {
                        args_path = convert_grid_location_into_filltext_args(i, path);
                        this.ctx.fillText('E', args_path[0], args_path[1]);
                    } 
                }   
        ...
        if (i == mid_grid_y) {
            if (this.west !== null) { // a room exists to north, paint a door.
                ...
                if (this.west.visible) {
                    for (path = this.location[0] - 1; path >= GRID_DIMENSIONS[this.level_room_index][0]; path--) {
            ...
            if (this.east !== null) {
                ...
                if (this.east.visible) {
                    for (path = this.location[0] + this.size[0] + 2; path < GRID_DIMENSIONS[this.level_room_index][0] + HOLDER_SIZE_X; path++) {
                        args_path = convert_grid_location_into_filltext_args(path, i);
                        this.ctx.fillText('E', args_path[0], args_path[1]);
                    }
                } 
    ...
    if (DEBUG_SHOW_ROOM_INFO) {
        ...
        args = convert_grid_location_into_filltext_args(location[0]+1, location[1]+1);
