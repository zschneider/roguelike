...
Level.prototype.generate_room_tree = function () {
  var start_room = this.add_start_room();
  // determine number of doors for starting room
  var doors = this._get_random_number_of_doors(0);
...
Level.prototype._continue_branching_rooms = function (
  rooms_to_be_explored,
  branch_level
) {
  ...
  if (curr_door_probs === undefined) {
    // Save some debug information
    for (i = 0; i < rooms_to_be_explored.length; i++) {
...
Level.prototype._create_connected_rooms = function (doors, old_room) {
  ...
  for (var i = 0; i < doors && cardinals.length !== 0; i++) {
    var r = getRandomInt(0, 3 - i);
    var door_to_be = cardinals[r];
    cardinals.splice(r, 1);
    var index = this.convert_cardinal_into_index(
      door_to_be,
      old_room.level_room_index
    );
...
Level.prototype._set_room_relationships = function (
  direction,
  new_room,
  old_room
) {
...
Level.prototype.assign_escape_room = function () {
  ...
  for (var i = 0; i < this.room_grid.length; i++) {
    if (
      this.room_grid[i] !== null &&
      this.room_grid[i] !== undefined &&
      this.room_grid[i].door_num == 1
    ) {
...
Level.prototype.assign_monsters = function () {
  for (var i = 0; i < this.room_grid.length; i++) {
    if (
      this.room_grid[i] !== undefined &&
      !this.room_grid[i].is_escape_room &&
      !this.room_grid[i].is_start_room
    ) {
...
Level.prototype.add_start_room = function () {
  ...
  // Setup the room
  start_room.set_size_and_placement(
    GRID_DIMENSIONS[start_room.level_room_index]
  );
...
Level.prototype.check_if_fit = function (index) {
  ...
  if (this.room_grid[index] !== undefined) {
    return false;
  }
  return true;
};
...
Level.prototype.check_space_for_monster = function (loc) {
  for (var i = 0; i < this.monsters.length; i++) {
    if (
      this.monsters[i].location[0] == loc[0] &&
...
Level.prototype.next_to_player = function (monster) {
  if (
    monster.location[0] + 1 == this.player.location[0] &&
    ...
    monster.location[0] == this.player.location[0] &&
...
Level.prototype._draw_health = function () {
  ...
  for (var i = 0; i < Math.ceil(this.player.health / 7); i++) {
    // this gets us 15 stars
...
Level.prototype._draw_mana = function () {
  ...
  for (var i = 0; i < Math.ceil(this.player.mana / 7); i++) {
    // this gets us 15 stars
...
Level.prototype._draw_border = function () {
  ...
  for (i = 1; i < this.c.height / 10 - 1; i += 1) {
    ...
    var args_right = convert_grid_location_into_filltext_args(
      this.c.width / 10 - 1,
      i
    );
